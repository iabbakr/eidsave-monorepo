...
type FeatherIconName = "list" | "package" | "message-circle" | "settings" | "log-out" | "chevron-right" | "award" | "copy";
...
export default function ProfileScreen() {
  ...
  const profileComplete = user?.profileComplete ?? 60;
  ...
  return (
    ...
    >
      ...
      {user?.savingsStreak != null && user.savingsStreak > 0 && (
        <View style={[styles.streakBadge, { backgroundColor: colors.accent + "15", borderColor: colors.accent + "30" }]}>
          ...
          <Text style={[styles.streakText, { color: colors.accent }]}>
            {user.savingsStreak} week saving streak
          </Text>
        </View>
      )}
      ...
      <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}>
        ...
        {user?.referralCode && (
          <View style={[styles.referralRow, { borderTopColor: colors.border }]}>
            <View>
              <Text style={[styles.referralLabel, { color: colors.mutedForeground }]}>Referral Code</Text>
              <Text style={[styles.referralCode, { color: colors.foreground }]}>{user.referralCode}</Text>
            </View>
            <Pressable style={[styles.copyBtn, { backgroundColor: colors.muted }]}>
      ...
      <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}>
        ...
        <ProfileRow label="Support" icon="message-circle" onPress={() => router.push("/support")} colors={colors} />
      ...
      <Text style={[styles.version, { color: colors.mutedForeground }]}>EidSave v1.0</Text>
    </ScrollView>
  );
}
